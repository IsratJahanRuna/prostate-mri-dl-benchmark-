# efficientnet_universal.py
"""
Universal EfficientNet-B0 trainer/evaluator for ADC, DWI, and T2 slice datasets
(extended to produce best.pt, patient-level CSVs, test summary, config, seed, splits, master metadata)

New artifacts per run (under out_dir/<DATASET>/<timestamp>/):
- best.pt
- splits.json
- prostate158_master.csv
- val_patients.csv
- val_roc_patients.csv
- test_patients.csv
...
# ----------------------
def main():
    ap = argparse.ArgumentParser(description="Universal EfficientNet-B0 for ADC/DWI/T2 (patient-level outputs)")
    ...
    # Training + best checkpoint by patient-level val AUC
    best_auc = -1.0
    ...
    for epoch in range(1, args.epochs+1):
        ...
        val_auc = roc_auc_score(df_val_pat['y_true'].values, df_val_pat['p'].values) \
                  if len(df_val_pat['y_true'].unique())==2 else float('nan')
    ...
    # ---- Final VAL patients + threshold_J ----
    val_prob, val_y, _, val_pid = collect_probs(model, val_loader, device)
    df_val_pat = aggregate_to_patients(val_pid, val_prob, val_y)
    ...
    roc_val_df.to_csv(out_base/'val_roc_patients.csv', index=False)
    ...
    test_auc = roc_auc_score(df_test_pat['y_true'].values, df_test_pat['p'].values) \
               if len(df_test_pat['y_true'].unique())==2 else float('nan')
    ...
    # ---- Config YAML ----
    cfg = {
        'run_name': f"efficientnet_{args.dataset}_seed{args.seed}",
        'seed': args.seed,
        ...
        'train': {'epochs': args.epochs, 'early_stop_patience': 0},  # best by val AUC
        'logging': {'out_dir': str(out_base.resolve())}
    }
    dump_yaml_like(cfg, out_base/'config_used.yaml')

    print(f"Run complete. Outputs in: {out_base}")
    print("Saved: best.pt, splits.json, prostate158_master.csv, val_patients.csv, val_roc_patients.csv, "
          "test_patients.csv, test_patient_preds.csv, test_eval.json, config_used.yaml, seed.txt")
